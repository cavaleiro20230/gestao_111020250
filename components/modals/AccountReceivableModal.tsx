...
interface AccountReceivableModalProps {
  ...
  onClose: () => void;
  onSave: (account: AccountReceivable, paymentDate: string) => void;
  costCenters: string[];
}

const AccountReceivableModal: React.FC<AccountReceivableModalProps> = ({ account, onClose, onSave, costCenters }) => {
  const [formData, setFormData] = useState<AccountReceivable>(
    account || { 
        id: uuidv4(), 
        description: '', 
        category: '', 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if(isFormValid) {
        onSave(formData, formData.paymentDate || new Date().toISOString().split('T')[0]);
        onClose();
    }
  };

  return (
    <Modal isOpen={true} onClose={onClose} title={account ? 'Editar Conta a Receber' : 'Nova Conta a Receber'}>
